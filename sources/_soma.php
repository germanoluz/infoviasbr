...
$consultaBr->causasDeOco($_POST["primeiro"]);
$consultaBr->trechoMaisPerigoso($_POST["primeiro"]);
?>
...
<script>
	$(function () {
		$('#grafbr').highcharts({
			...
			title: {
				text: 'Dados estatísticos da BR <?php echo $_POST["primeiro"]; ?>'
			},
			tooltip: {
		...
		<div class="row half">
			<div id="infobr" class="4u">
				...
				<h3>Trecho mais perigoso:</h3>
					<span class="results icon-eye-open"> <?php echo "Km: " . $consultaBr->getTrecho() . "<br>Qtd de acidentes: " . $consultaBr->getQtdTrecho(); ?></span>
				<h3>Principal causa de acidente:</h3>
					<span class="results icon-exclamation-sign"> <?php echo $consultaBr->getCausa() . ": " . $consultaBr->getQtdCausa(); ?></span>
				<h3>Veículo envolvido mais comum:</h3>
					<span class="results icon-ambulance"><!-- Resultado da consulta do tipo de veículo mais comum em acidente --></span>
				<h3>Acidentes por alcoolismo:</h3>
					<span class="results icon-glass"><!-- Resultado da consulta do número de acidentes por alcoolismo --></span>
			</div>
